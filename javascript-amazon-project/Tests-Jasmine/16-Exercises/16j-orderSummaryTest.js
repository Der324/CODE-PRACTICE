...
describe('test suite: renderOrderSummary', () => {
  ...
  it ('displays the cart', () => {
    ...
    ).toBe(`$${formatCurrency(product1.priceCents)}`);

    expect(
      document.querySelector(`.js-product-price-${productId2}`).innerText
    ).toBe(`$${formatCurrency(product2.priceCents)}`);
  ...
  it('removes a product', () =>{
    ...
    expect(
      remainingItem.querySelector(`.js-product-price-${productId2}`).innerText
    ).toBe(`$${formatCurrency(product2.priceCents)}`);
  ...
  it('updates the delivery option', () => {
    ...
    const thirdDeliveryOption = document.querySelector(
    ...
    const shippingPrice = document.querySelector('.js-payment-summary-shipping').innerText;
    const totalPrice = document.querySelector('.js-payment-summary-total').innerText;

    expect(shippingPrice).toBe('$14.98')
    expect(totalPrice).toBe('$63.50');
  });
});